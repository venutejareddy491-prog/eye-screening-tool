...
export function calculateRiskClient(answers) {
  let score = 0;
  ...
  return { riskScore: score, riskPercentage: 0, riskCategory };
}

export function getRecommendations(riskCategory, answers) {
  const base = [
    { title: '20-20-20 Rule', text: 'Every 20 minutes, look at something 20 feet away for 20 seconds.', icon: 'clock' },
    { title: 'Screen Breaks', text: 'Take a 5-minute break every hour. Blink consciously during breaks.', icon: 'monitor' },
    { title: 'Hydration', text: 'Aim for 6–8 glasses of water daily.', icon: 'droplet' },
  ];
  const moderate = [
    { title: 'Sleep Hygiene', text: 'Target 7–8 hours of sleep. Keep screens off 1 hour before bed.', icon: 'moon' },
    { title: 'Humidify Your Space', text: 'Use a humidifier if AC dries the air (40–50% humidity).', icon: 'wind' },
    { title: 'Artificial Tears', text: 'Consider preservative-free lubricating drops 2–4 times daily.', icon: 'eye' },
  ];
  const high = [
    { title: 'See an Eye Specialist', text: 'Schedule a comprehensive eye exam soon.', icon: 'stethoscope', urgent: true },
    { title: 'Reduce Contact Lens Wear', text: 'Limit lens hours on high-symptom days.', icon: 'glasses' },
    { title: 'Smoking Cessation', text: 'Smoking worsens dry eye significantly.', icon: 'alert' },
  ];
  let recs = [...base];
  ...
  if (answers?.screenTime === 'over12' || answers?.screenTime === '8-12') {
    recs.push({ title: 'Blue Light Awareness', text: 'Enable night mode and consider blue-light glasses.', icon: 'shield' });
  }
  return recs;
}
...
export function getRiskFactorBreakdown(answers) {
  ...
  const add = (name, value, weight) => {
    if (value && weight > 0) factors.push({ name, value, weight });
  };
...
export function buildLocalScreening(answers) {
  const { riskScore, riskPercentage, riskCategory } = calculateRiskClient(answers);
  return {
    answers,
    symptoms: {
    ...
    riskPercentage,
    riskCategory,
    recommendations: getRecommendations(riskCategory, answers),
    riskFactors: getRiskFactorBreakdown(answers),
...
export function riskColor(category) {
  if (category === 'High Risk') return { bg: 'bg-red-500', text: 'text-red-600', ring: 'ring-red-500/30', gradient: 'from-red-500 to-orange-500' };
  if (category === 'Moderate Risk') return { bg: 'bg-amber-500', text: 'text-amber-600', ring: 'ring-amber-500/30', gradient: 'from-amber-500 to-yellow-500' };
  return { bg: 'bg-emerald-500', text: 'text-emerald-600', ring: 'ring-emerald-500/30', gradient: 'from-emerald-500 to-teal-500' };
}
